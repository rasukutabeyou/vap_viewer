...
- **P(SHIFT)パネル**: 常に「上=SHIFT予測 / 下=HOLD予測」になるよう正規化した曲線。
  ...
  (次話者視点)のままです。score曲線と合わせて**他のパネルより高く**描画します。
  閾値は 0.1 前後で評価窓の中の差が数pxしか出ないことがあるため、サイドバーの
  「表示パネル」に **「P(SHIFT) を評価窓の判定域まで拡大」**(既定OFF)があります。
  ONにすると評価窓の中の曲線と**全モデルの閾値**が収まる範囲までY軸を拡大します
  (窓の外のピークは切れます。ケース間で高さを見比べたいときはOFFのまま)。
